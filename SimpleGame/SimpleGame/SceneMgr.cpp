...
void SceneMgr::IsCollide(int num) {
	...
	for (int i = 0; i < push_count; ++i) {
		...
		if (num != i) {
			...
			bool g = (m_ob[num]->GetType() == 1 && m_ob[i]->GetType() == 3) || (m_ob[num]->GetType() == 3 && m_ob[i]->GetType() == 1);
...
void SceneMgr::UpdateObj(float time) {
	...
	for (int i = 0; i < push_count; ++i) {
		...
		if (m_ob[i]->GetType() == 1 &&m_ob[i]->GetLife()>0&&bull_count%100==0) {
			...
			AddActorObject(m_ob[i]->GetPosX() - (100), m_ob[i]->GetPosY() - 100, 3);
		}
		IsCollide(i);
...
void SceneMgr::AddActorObject(float a, float b, int o) {
	if (push_count < MAX_OBJECTS_COUNT) {
		if (o == 1)
			...
			m_ob[push_count] = new Object(a, b, 0, 1.0f, 0.0f, 0.0f, 2, 6, 20,o);
		else if (o == 4)
			m_ob[push_count] = new Object(a, b, 0, 0.0f, 1.0f, 0.0f, 2, 100, 100,o);
		push_count++;
	}
		...
		for (int i = 0; i < push_count; ++i) {
			if (dead_ob[i] == 1) {
				if (o == 1) {
					...
					if(rand()%2==1)
						m_ob[i] = new Object(a, b, 0, 1.0f, 0.0f, 0.0f, 2, 6, 20, o);
					else
						m_ob[i] = new Object(a, b, 0, 1.0f, 0.0f, 0.0f, 2, -6, 20, o);
					dead_ob[i] = 0;
				}
				else if (o == 4){
					m_ob[i] = new Object(a, b, 0, 0.0f, 1.0f, 0.0f, 2, 100, 100, o);
					dead_ob[i] = 0;
				}
				return;
			}
